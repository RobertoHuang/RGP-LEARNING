...
- `Replica B`是当前的`leader`副本，`Replica A`是当前的`follower`副本。在某一时刻`B`中有两条消息`m1`和`m2`，`A`从`B`中同步了这两条消息此时`A`和`B`的`LEO`都为2同时`HW`都为1，之后`A`再向`B`中发送请求拉取消息，`FetchRequest`请求带上了`A`的`LEO`信息，`B`在收到请求之后更新自己的`HW`为2，`B`中虽然没有更多的消息但还是在延时一段时间后返回`FetchResponse`，并在其中包含了`HW`的信息，最后`A`根据`FetchResponse`中`HW`信息更新自己的`HW`为2
  ...
  <img align="center" src="https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1%E5%9C%BA%E6%99%AFA.png" />
  ...
  ![数据丢失场景](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1%E5%9C%BA%E6%99%AFB.png)
  ...
  ![数据丢失场景](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1%E5%9C%BA%E6%99%AFC.png)
...
- 当前`Leader`副本为`A`，`follower`副本为`B`，`A`中有2条消息`m1`和`m2,`并且`HW`和`LEO`都为2，`B`中有一条消息`m1`并且`HW`和`LEO`都为1,。假设`A`、`B`同时挂掉然后`B`第一个恢复过来并成为`leader`如图所示
  ...
  ![数据不一致](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E6%95%B0%E6%8D%AE%E4%B8%8D%E4%B8%80%E8%87%B4A.png)
  ...
  ![数据不一致](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E6%95%B0%E6%8D%AE%E4%B8%8D%E4%B8%80%E8%87%B4B.png)
...
- `Leader Epoch`解决数据丢失问题
  ...
  ![解决数据丢失](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1A.png)
  ...
  ![解决数据丢失](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1B.png)
  ...
  ![解决数据丢失](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%A2%E5%A4%B1C.png)
  
- `Leader Epoch`解决数据不一致问题

  当前`A`为`leader`，`B`为`follower`，`A`中有两条消息`m1`和`m2`，而`B`中有一条消息`m1`。假设`A`和`B`同时挂掉然后`B`第一个恢复过来并成为新`leader`

  ![解决数据不一致](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%8D%E4%B8%80%E8%87%B4A.png)

  之后`B`写入消息`m3`，并将`LEO`和`HW`更新至2，注意此时的`LeaderEpoch`已经从`LEO`增至`LE1`了
  
  ![解决数据不一致](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%8D%E4%B8%80%E8%87%B4B.png)
  ...
  ![解决数据不一致](https://raw.githubusercontent.com/RobertoHuang/RGP-LEARNING/master/Kafka/images/%E5%BA%94%E5%AF%B9%E6%95%B0%E6%8D%AE%E4%B8%8D%E4%B8%80%E8%87%B4C.png)
### 为什么不支持读写分离
