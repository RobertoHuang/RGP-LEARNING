...
- 数据卷
    ...
    docker run --name=nginx -d -p 80:80 -p 443:443 -v /usr/local/docker/nginx/html:/usr/local/nginx/html -v /usr/local/docker/nginx/logs:/usr/local/nginx/logs -v /usr/local/docker/nginx/nginx.conf:/usr/local/nginx/conf/nginx.conf 201210704116/nginx:1.0
    ```
